...
const Profile = () => {
  ...
  const edit_data = (
    <input
      type={edit_data_type}
      defaultValue={values[chosen_span]}
      onChange={(e) => {
        if (e.target.value !== undefined) new_value = e.target.value;
      }}
    />
  );
    ...
    return (
      <>
        <tr>
          <td>{lable} </td>
          <td>{data[field]}</td>
          <td>{edit_btn(field, type, link)}</td>
        </tr>
      </>
    );
  };
  const change_value = () => {
    ...
    case 'fatherName':
      show_invalid(
        new RegExp('^[a-zA-Zа-яА-Я]+$').test(new_value) &&
          new_value !== undefined
      );
      break;
    default:
    ...
    return (
      <input
        ...
        onClick={() => {
          ...
          set_chosen_span(field);
          set_edit_data_type(type);
        }}
      ></input>
    );
  };
